...
void F1(){
    ...
    std::cout << "Введите текст в формате \"время единица_измерения единица_измерения\":\n";
...
std::istream& operator>> (std::istream& is,Matrix& mat){
    ...
    for(size_t a = 0; a != mat.Size().first; ++a){
        ...
        if(parsed_line.size() != mat.Size().second){
            throw std::logic_error("количество введенных в строчке элементов не равно размеру матрицы");
        }
...
void F4(){
    std::cout << "Напишите размер матриц и ниже, рязделяя числа пробелами сами матрицы:\n";
    size_t N;
    std::cin >> N;
    std::cin.ignore();
